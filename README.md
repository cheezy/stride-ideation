...
|---|---|---|
| `lean` (default) | Engineering-only audience; small topic; you want the shortest path to a committed doc | No augmentations — runs the shared core only: the seven hard-gated sections plus the mandatory round-3 framing, round-4 premortem, and challenge gate, with no extra forcing questions, no optional document sections, and no profile-specific reviewer checks |
