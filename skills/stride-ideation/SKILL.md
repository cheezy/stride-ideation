...
The recap is **display-only narration**. It MUST NOT change the gate, the round order, or the question budget, and it is NEVER an extra `AskUserQuestion` round — it is printed immediately before the round's question batch, then the round proceeds normally.
