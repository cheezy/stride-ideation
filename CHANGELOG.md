...
### Added — batch validator scored-field warnings and length-limit checks (W1467)

Two new client-side checks in `lib/validate_batch.py` catch defect classes that previously sailed through local validation and failed opaquely (or invisibly) in production. A new fatal check `(f) length_limit` fails validation with the offending JSON path and length when a goal/task `title` or any `security_considerations` element exceeds 255 Unicode code points — exactly the fields the server binds to `varchar(255)`; `pitfalls` elements and `key_files` notes are deliberately excluded because they are JSONB (unbounded) on the server, so checking them would reject batches the API accepts. After all fatal checks pass, an advisory scored-field completeness pass warns on stdout (never changing the exit code) for any task whose review-queue scored field is missing or empty — surfacing empty-pill risk before the POST instead of after the goal ships; warnings go to stdout because stridify's Step 8a captures only stderr. `lib/test-validate-batch.sh` doubled from 15 to 30 assertions (multibyte CJK tests pin code-point-not-byte counting; missing-key vs empty-array vs whitespace-string warning shapes pinned; an ordering test proves warnings never precede a fatal exit; all three repo fixtures assert silent passes), and stridify.md's Step 8a documents the six checks plus the advisory pass accurately.
...
### Fixed — calibration fixtures now carry the five scored fields on every task (W1462)

The three fixture batch JSONs are the calibration reference for good decomposer output, yet none of their 35 tasks carried `testing_strategy` or `security_considerations`, and the notifications-system and replace-test-suite fixtures additionally omitted `pitfalls`, `patterns_to_follow`, and task-level `why`/`what`/`where_context` — anyone calibrating from them reproduced the empty-pill omission. Every task in all three fixtures now carries the five review-queue scored fields with scenario-realistic values (substantive security entries where a real surface exists — signed unsubscribe tokens, SMTP header-injection, PII in payloads — and the none-with-reason convention for measurement and styling tasks), and the two thin fixtures gained per-task `why`/`what`/`where_context` to match the dark-mode fixture's richness. Changes are strictly additive: task counts, titles, dependencies, and `source_spec`/`source_spec_sha256` are byte-identical; all three fixtures pass `lib/validate_batch.py`; the smoke test stays green (14/0). `fixtures/README.md` documents the new five-field invariant and clarifies that `source_spec_sha256` hashes the requirements doc, so field-only batch edits need no SHA bump.

### Added — created_by_agent stamped on shipped goals in the stridify pipeline (W1461)

Goals shipped through `/stridify` never carried `created_by_agent`, so the Stride agents activity feed attributed them to nobody — and because the field is create-only (forbidden on PATCH), every batch shipped without it was permanently unattributed. Step 8b now stamps `created_by_agent` on **every goal object** (the server propagates the goal value to nested child tasks) with the running agent's plain name, per the canonical creating-goals rule — the exact same value sent as `agent_name` on claim and complete. The stamp happens before the write/commit, so the committed batch artifact and the POST body both carry it identically; unlike the three root audit fields it is **never stripped** (`lib/strip_audit_fields.py` gains a guard comment forbidding it from the strip set, and `lib/test-stamping.sh` gains a two-goal stamp-then-strip round-trip regression proving survival plus audit-field removal). This fulfills the "stamps it at ship time" promise in the decomposer's do-not-emit list.
