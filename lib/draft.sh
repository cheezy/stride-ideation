...
# ideation draft (answered sections + round state) to a gitignored scratch
# file under .stride/, so an interruption mid-session is recoverable and a
# later /ideate run can offer to resume it:
#
