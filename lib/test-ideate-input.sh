...
# documented in commands/ideate.md (W1137). The command body runs inside a
# live Claude Code session, so this test embeds reference shell
# implementations of the documented Step 1 --input parse, the file-exists
# validation, and the Step 4c read-only invariant, and exercises them.
#
# The reference implementations below MUST stay consistent with Step 1 (the
# --input parse + validation) and Step 4c (the read-only seed read) in
# commands/ideate.md. If you edit one, edit both — this test exists to prevent
# the doc and the on-the-wire behavior from drifting apart.
#
# Run:
...
#
# Mirrors commands/ideate.md Step 1: parse --continue and --input (each in
# both `--flag <value>` and `--flag=<value>` forms), consuming their tokens;
...
# (--profile parsing is covered by test-stridify-per-goal.sh's sibling parser
...
#
# Mirrors commands/ideate.md Step 1's INPUT_PATH existence check: unset is OK
# (no seed); a set-but-missing path is a one-line error + non-zero exit; an
# existing regular file is OK.
...
#
# Mirrors commands/ideate.md Step 4c: read the file read-only into a variable.
# It MUST NOT write, move, or modify the file in any way.
...
# The notes file must not have been moved/renamed or git-tracked side-effects.
if [ -f "$NOTES" ]; then
